...
export const CitizenDashboard: React.FC<CitizenDashboardProps> = ({
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Ward 14 (Sector 14) • Citizen Reliability Index:{' '}
        ...
        <button
          ...
          className="px-4 py-2.5 rounded bg-teal-800 hover:bg-teal-900 text-white text-xs font-semibold flex items-center gap-1.5 shadow-xs shrink-0"
        >
      ...
      {/* 2. OVERVIEW METRICS (Section 4 Specs) */}
      ...
      <div className="flex items-center space-x-2 border-b border-slate-200 pb-2 text-xs">
        ...
        >
          Nearby Civic Issues & Map ({issues.length})
        </button>
      </div>

      {/* 4. TAB 1: MY REPORTS TABLE */}
      {activeTab === 'my_reports' && (
        <div className="bg-white rounded-lg border border-slate-200 overflow-hidden shadow-xs">
          <div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500">Click any row to open tracking timeline</span>
          </div>
                ...
                {myReports.map(issue => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-right" onClick={e => e.stopPropagation()}>
                      <button
                        onClick={() => onOpenTrack(issue.id)}
                        className="px-2.5 py-1 rounded bg-slate-100 hover:bg-slate-200 text-slate-800 font-medium text-[11px] border border-slate-200"
                      >
                        Timeline →
                      </button>
                    </td>
      ...
      {/* 5. TAB 2: NEARBY CIVIC ISSUES & MAP (Section 4 & 5 Specs) */}
      {activeTab === 'nearby_map' && (
