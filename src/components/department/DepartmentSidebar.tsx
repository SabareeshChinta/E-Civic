...
export const DepartmentSidebar: React.FC<DepartmentSidebarProps> = ({
  ...
  return (
    <aside className="w-full md:w-64 bg-slate-900 text-slate-300 flex flex-col justify-between shrink-0 md:min-h-[calc(100vh-3.5rem)] border-b md:border-b-0 md:border-r border-slate-800">
      {/* Top Header / Mobile Bar */}
      <div className="p-3 md:p-4 space-y-3 md:space-y-6">
        {/* Top Header Label (Hidden on small mobile to save space) */}
        ...
        <nav className="flex md:flex-col space-x-1 md:space-x-0 md:space-y-1 text-xs overflow-x-auto no-scrollbar pb-1 md:pb-0">
          {navItems.map(item => {
            ...
            return (
              <button
                ...
                className={`flex items-center justify-between px-3 py-2 md:py-2.5 rounded text-left transition font-medium shrink-0 ${
                  isActive
              ...
              >
                ...
                {item.badge !== undefined && item.badge > 0 && (
                  <span
                    className={`ml-1.5 text-[10px] font-mono px-1.5 py-0.2 rounded font-bold ${
      ...
      {/* Footer Officer Profile Info (Desktop Only) */}
      <div className="hidden md:block p-4 border-t border-slate-800 bg-slate-950/60 text-xs">
